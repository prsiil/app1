...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER main Purpose: {purpose};
    TASK: Write a product description that is tailored into this customer's Age group and purpose. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and purpose;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account purpose {purpose} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...";
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "purpose", "content"],
    template=template,
)
...
with col2:
    st.image(image='ratas.jpg', caption='Natural and healthy shirts for everybody')
...
def get_hobby():
    input_text = st.text_input(label="Customers ride purpose", key="hobby_input")
    return input_text
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, purpose=hobby_input, content=content_input)
